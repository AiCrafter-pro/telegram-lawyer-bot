...
import { createMcpTransport, toGeminiTools, toFunctionResponse, callMcpTool } from './mcp.js';
...
export async function initMCP() {
    ...
    geminiTools = toGeminiTools(toolsResult.tools);
}
    ...
    while (response.functionCalls && response.functionCalls.length > 0 && toolRound < MAX_TOOL_ROUNDS) {
        ...
        for (const call of allCalls) {
            ...
            if (call.name === 'read_website_url') {
                ...
                functionResponses.push(toFunctionResponse(call, { result: textData }));
                continue; // 아래 로직 건너뛰기
            }
            ...
            // 인증·재시도는 MCP가 처리합니다. 별도 민법 조회를 매번 중복 실행하지 않습니다.
            const toolResponse = await callMcpTool(mcpClient, call);

            const toolElapsed = Date.now() - toolStart;
            console.log(`  [🐾 ${call.name}] 파라미터: ${JSON.stringify(call.args)}`);

            if (toolResponse.isError) {
                console.warn('  [MCP] 법률 도구 오류: ' + call.name);
                functionResponses.push(toFunctionResponse(call, toolResponse));
                continue;
            }
            const mcpDataString = toolResponse.result;
